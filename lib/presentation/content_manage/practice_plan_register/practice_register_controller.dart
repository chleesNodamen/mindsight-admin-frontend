...
class PracticeRegisterController extends GetxController {
  ...
  Future<void> saveChanges() async {
    practicesDataModel = await PracticesDataRepository().post(
        PracticesDataReqPost(bodyId: selectedBodyId, breathId: selectedBreathId)
            .toJson());
    if (practicesDataModel.isSuccess) {
      if (Get.isRegistered<PracticePlanManageController>()) {
        Get.delete<PracticePlanManageController>();
      }
      Get.toNamed(AppRoutes.contentPracticePlanManage);
    }
  }
