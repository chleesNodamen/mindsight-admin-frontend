...
class ChangePassword1Controller extends GetxController {
  ...
  Future<void> onResendCode() async {
    ...
    BaseModel model = await MasterVerifyEmailRepository().post(
        MasterVerifyEmailReqPost(
            email: email, type: MasterVerifyEmailRepository.resetPassword));
  ...
  Future<void> onContinue() async {
    ...
    if (model.isSuccess) {
      Get.toNamed(AppRoutes.changePassword2, arguments: {
        RouteArguments.email: email,
        RouteArguments.code: codeController.text
      });
    }
  }
}
